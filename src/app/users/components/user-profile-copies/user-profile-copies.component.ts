...
export class UserProfileCopiesComponent implements OnInit {
  ...
  showCopy( copyId: string, bookId: string): void {
    ...
    if (this.userId) {
      this.router.navigate(['/world', 'users', this.userId, 'copies', 'view', copyId, 'books', bookId]).then();
    }

    if (this.followingId) {
      this.router.navigate(['/world', 'following', this.followingId, 'copies', 'view', copyId, 'books', bookId]).then();
    }

    if (this.followerId) {
      this.router.navigate(['/world', 'follower', this.followerId, 'copies', 'view', copyId, 'books', bookId]).then();
    }
  }
